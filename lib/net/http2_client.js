...
async function http2_fetch(session, headers, options) {
  ...
  const request_error_promise = new Promise((reject) =>
  ...
  request_stream?.on("data", (chunk) => (data += chunk))
  ...
  try {
    await Promise.race([end_promise, request_error_promise])
  } catch (err) {
...
async function https_fetch(domain, headers) {
  ...
  const options = {
    ...
    host: domain.substring(domain.indexOf("/") + 2),
    // @ts-ignore
    path: headers[":path"],
    headers: {
...
export async function fetch(domain, headers, options) {
  ...
  try {
    session = await new Promise((resolve, _) =>
