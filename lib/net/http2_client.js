...
export async function fetch(
  ...
  // NOTE Unknown if connect with work with a full url
  try {
    /** @type {import("http2").ClientHttp2Session} */
    var session = await new Promise((resolve, _) =>
      connect(parsed_url, (session) => resolve(session))
    )
  } catch (err) {
    return Promise.reject(err)
  }
  if (!session.alpnProtocol) {
    session.close()
  }
