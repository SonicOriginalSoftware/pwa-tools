...
export class LocalFile {
  ...
  constructor(buffer, start_offset) {
    ...
    console.log(`Raw buffer start index: ${raw_buffer_start}`)
    console.log(`Raw buffer should have length: ${this.#compressed_size}`)
    this.#raw_buffer = buffer.slice(
      start_offset + this.#size,
      this.#compressed_size
    )
    console.log(`Actual raw buffer length: ${this.#raw_buffer.length}`)
    console.log(`Actual raw buffer byte-length: ${this.#raw_buffer.byteLength}`)
    console.log(`Raw buffer: ${this.#raw_buffer}`)
    if (this.#raw_buffer.byteLength !== this.#compressed_size) {
      throw new RangeError("Raw buffer length is not equal to compressed size!")
    }
  }
    ...
    switch (this.#compression_method) {
      case COMPRESION_METHOD.DEFLATED:
        return new Promise((resolve, reject) =>
          deflate(this.#raw_buffer, (err, decompressed_buffer) =>
