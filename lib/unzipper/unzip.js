 ...
 * @param {EndOfCentralDirectory} end_of_central_directory
 */
async function generate_structure(buffer, end_of_central_directory) {
  ...
  for (const each_central_directory of central_directories) {
    ...
    const content = (await local_file.content).toString("utf-8")
...
export async function unzip(source, target_path) {
  ...
  var structure = await generate_structure(source, headers.end_of_central_directory)
