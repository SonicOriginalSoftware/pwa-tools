...
export async function init(
  ...
  const [pwa_server_response] = await Promise.all([
  ...
  await Promise.all([
    // extract(pwa_shell_response.data, target_directory + "/pwa-shell/", {
    //   encoding: archive_encoding,
    // }),
    extract(pwa_server_response.data, target_directory + "/pwa-server/", {
      enc: archive_encoding,
    }),
  ])
