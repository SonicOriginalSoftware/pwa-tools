...
async function move_files(root_start_directory, exclusion_list = []) {
  ...
  if (package_root_path === undefined)
    return Promise.reject(`No package root found in ${root_start_directory}!`)
...
async function run() {
  ...
  if (!(await destination_directory_clean(shell_destination)))
    return Promise.reject("Shell extraction folder not empty!")
  if (!(await destination_directory_clean(server_destination)))
