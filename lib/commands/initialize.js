...
export const usage = `
  Usage:
  npx <node-options> @sonicoriginalsoftware/pwa-tools init help
  npx <node-options> @sonicoriginalsoftware/pwa-tools init * --cache -t ./new-site
  npx <node-options> @sonicoriginalsoftware/pwa-tools init . lib/indexedDb --cache -t ./new-site
  npx <node-options> @sonicoriginalsoftware/pwa-tools init https://your-domain.com/path/to/your/archive.zip components/your-component
  ...
  if (subcommand === "*") {
    return Promise.all([
      ...
      run(
        ...
        ["package.json", "vscode.code-workspace", "tsconfig.json", "README.md"],
        options.cache
      ),
    ])
  } else {
